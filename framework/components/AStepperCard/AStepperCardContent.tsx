...
const AStepperCardContent = forwardRef<HTMLDivElement, AStepperCardContentProps>(
  ({className: propsClassName = "", children, title, subtitle, nextButtonText = "Next", backButtonProps, cancelButtonProps, nextButtonProps, contentFooter, ...rest}, ref) => {
    const baseClass = "a-stepper-card__content";
    return (
        <ACol cols="8" className={classnames(`${baseClass} ${propsClassName} d-flex flex-column`)} style={{height: "100%"}} ref={ref} {...rest as AColProps}>
          ...
          <ACardContent className="overflow-y-scroll ma-4 mb-0 pb-4">{children}</ACardContent>
            {contentFooter ? contentFooter : <ACardFooter className={classnames(`${baseClass}--footer`, "justify-space-between py-3")}>
<ACardItem attached="right">
              {  cancelButtonProps && <AButton medium tertiary {...cancelButtonProps}>
                Cancel
              </AButton>}
            </ACardItem>
            <ACardItem attached="right" >
              ...
              <AButton medium {...nextButtonProps}>{nextButtonText}</AButton>

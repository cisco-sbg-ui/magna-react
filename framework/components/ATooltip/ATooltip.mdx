...
<AAlert dismissible={false} level="danger">
...
<Playground
  ...
  return (
    <>
      ...
      <p>
        To align the button with the secondary axis specifier (-left, -right), use 'alignPointerOnSide'
      </p>
      <AButton secondary ref={buttonRef3} onClick={() => setOpen3(!open3)}>
      Side aligned pointer
      </AButton>
      <ATooltip
        ...
        pointer
        alignPointerOnSide>
         Tooltip pointer is placed on the left edge of the tooltip and anchor alignment
      </ATooltip>
    </>
  );
}

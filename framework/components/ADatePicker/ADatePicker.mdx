---
name: Date Pickers
route: /components/date-picker
components: ADatePicker, ADateRangePicker
title: ADatePicker
sourceCodeLink: https://github.com/cisco-sbg-ui/magna-react/tree/canary/framework/components/ADatePicker
---

## Usage

### Import

<ComponentImport components="ADatePicker, ADateRangePicker" />

### Usage

<Playground
  code={`() => {
  const [date, setDate] = useState(new Date(2021, 0, 14));
  return (
    <ADatePicker value={date} onChange={(date) => setDate(date)} />
  );
}
`}
/>

#### With Minimum and Maximum Dates

<Playground
  code={`() => {
  const earliestDay = new Date(2022, 2, 13);
  const latestDay = new Date(earliestDay);
  latestDay.setDate(earliestDay.getDate() + 13);
  const [date, setDate] = useState(earliestDay);
  return (
    <ADatePicker value={date} onChange={(date) => setDate(date)} minDate={earliestDay} maxDate={latestDay}/>
  );
}
`}
/>

#### Date Range

<Playground
  code={`() => {
  const { value, onChange, } = useADateRange();
  return (
    <ADatePicker value={value} onChange={onChange} />
  );
}
`}
/>

#### Date Range with Initial Range

<Playground
  code={`() => {
  const { value, onChange, } = useADateRange([new Date(2022, 2, 28), new Date(2022, 3, 5)]);
  const [firstSelection, secondSelection] = value;
  // You can destructure the values from useADateRange to use ...
  // ... the date picker's range value in your component
  return (
    <>
      <ADatePicker value={value} onChange={onChange} />
        <p aria-live="polite">
          Selected range: <i>{firstSelection && firstSelection.toLocaleDateString()} - {secondSelection && secondSelection.toLocaleDateString()}</i>
        </p>
    </>
  );
}
`}
/>

#### Date Range with Maximum Days

<Playground
  code={`() => {
  const { value, ...datePickerProps } = useADateRange({
    initialRange: [new Date(2022, 2, 14), new Date(2022, 2, 16)],
    maxDays: 3
  });
  const [firstSelection, secondSelection] = value;
  // Alternatively, you can spread out the remaining values ...
  // ... to be passed to the date picker
  return (
    <>
      <ADatePicker value={value} {...datePickerProps} />
      <p aria-live="polite">
        Selected range: <i>{firstSelection && firstSelection.toLocaleDateString()} - {secondSelection && secondSelection.toLocaleDateString()}</i>
      </p>
    </>
  );
}
`}
/>

#### Date Range with inputs provided

(yyyy year format required)

<Playground
  code={`() => {
  const {dates, getDateRange} = useGetADateRange();
  const [startDate, endDate] = dates;
return (

<>
  <ADateRangePicker
    label="Select range"
    getDateRange={getDateRange}
    initialRange={[new Date(2022, 2, 28), new Date(2022, 3, 5)]}
  />
  {startDate && (
    <p aria-live="polite">
      Selected range:
      <i>
        {startDate.toLocaleDateString()} -{" "}
        {endDate && endDate.toLocaleDateString()}
      </i>
    </p>
  )}
</>
); } `} />

## Component Props

### Date Picker Props

The `ADatePicker` component inherits passed props.

<Props of="ADatePicker" />

### Date Range Picker Props

The `ADateRangePicker` component inherits passed props.

<Props of="ADateRangePicker" />
